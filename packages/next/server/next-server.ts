...
import type { EdgeFunctionResult } from './edge-functions/types'
    ...
    for (const middleware of this.middleware || []) {
      if (middleware.match(params.parsedUrl.pathname)) {
        ...
        result.promise.catch((error) => {
          console.error(`Middleware handler failed`, error)
        })

        result.waitUntil.catch((error) => {
          console.error(`Promise failed after middleware response`, error)
        })
