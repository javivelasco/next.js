...
export default class Server {
  ...
  protected async runMiddleware(params: {
    ...
    const prevCalls = params.prevCalls || 0
    if (prevCalls > 5) {
    ...
    for (const middleware of this.middleware || []) {
      if (middleware.match(url.pathname)) {
        ...
        const calls = parseInt(
          result?.response.headers?.get('x-middleware-count') || '0',
          10
        )
        result.response.headers.set('x-middleware-count', `${calls + 1}`)
        ...
        if (result.response.headers.has('x-middleware-effect')) {
          ...
          if (rewrite?.startsWith('/')) {
            ...
            if (parsed.pathname !== req.url.pathname) {
              if (
                this.middleware?.some(({ match }) => match(parsed.pathname))
              ) {
                result.response.headers.delete('x-middleware-effect')
                const nextResult = await this.runMiddleware({
                ...
                if (nextResult?.response.headers.get('x-middleware-effect')) {
                  return nextResult
                }
              }
            }
