...
const MIDDLEWARE_FULL_ROUTE_REGEX = /^pages[/\\]?(.*)\/_edge$/
...
export default class MiddlewareManifestPlugin {
  ...
  createAssets(compilation: any, assets: any) {
    ...
    for (const entrypoint of entrypoints.values()) {
      ...
      if (isWebpack5 && !this.dev) {
        middlewareManifest.middleware[
          location
        ].file = middlewareManifest.middleware[location].file.slice(3)
      }

      middlewareManifest.middleware[
        location
      ].file = middlewareManifest.middleware[location].file.replace(/\\/g, '/')
    }
    ...
    assets[
      `${isWebpack5 && !this.dev ? '../' : ''}` + MIDDLEWARE_MANIFEST
